...
# 파일 발송 관련 함수들
def send_document(chat_id, document, reply_to=None, keyboard=None):
    u"""send_document: 파일 발송
    chat_id:    (integer) 파일를 보낼 채팅 ID
    text:       (string)  파일 내용
    reply_to:   (integer) ~파일에 대한 답장
    ...
    try:
        urllib2.urlopen(BASE_URL + 'sendDocument', urllib.urlencode(params)).read()
...
def get_document(msg) :
    ...
    if (not get_enabled(chat_id)):
        return
    cmd_echo(chat_id, 'document', reply_to=msg_id)

        
    document_id = document['file_id']
    try:
        document_file = urllib2.urlopen(BASE_URL + 'getFile', urllib.urlencode(document_id)).read()

        name = document_file.download("")

        send_document(chat_id, document=open(name, 'rb'))
    except Exception as e: 
        logging.exception(e)
    return
